...
import { QUESTION_CONTROL_TYPE } from 'src/app/_config/enum.config';
import { QuestionBaseModel } from '../models/question-base.model';
import {
  DropdownControl,
  TextboxControl,
} from '../models/question-control.model';
...
export class QuestionControlService {
      ...
      switch (question.controlType) {
        case QUESTION_CONTROL_TYPE.DYNAMIC_TEXT_BOX: {
          ...
          question.value.forEach((value: any) => {
            group[question.key].push(new FormControl(value, { validators }));
          });
          break;
        }
  ...
  validatePasswordMatch(
    ...
    const settingControl = this.getSettingControl(questions, 'confirmPassword');
  ...
  getSettingControl(questions: QuestionBaseModel<any>[], key: string) {
  ...
  checkValidate(formGroup: FormGroup, questions: QuestionBaseModel<any>[]) {
  ...
  getQuestions() {
    const questions: QuestionBaseModel<string>[] = [
      new DropdownControl({
        key: 'brave',
